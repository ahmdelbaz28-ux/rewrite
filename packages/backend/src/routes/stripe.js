...
async function handleCheckoutCompleted(session) {
  ...
  if (license) {
    ...
    ).run(tier, customerId, subscriptionId, JSON.stringify(TIER_FEATURES[tier]), maxDevices, expiresAt, Date.now(), license.id);
    ...
    ).run(token, tier, email, customerId, subscriptionId, JSON.stringify(TIER_FEATURES[tier]), maxDevices, expiresAt, Date.now(), Date.now());
...
async function handleSubscriptionUpdated(subscription) {
  ...
  if (subscription.status === 'active') {
    ...
    db().prepare('UPDATE licenses SET tier = ?, revoked = 0, updated_at = ? WHERE id = ?').run(tier, Date.now(), license.id);
...
async function handleSubscriptionDeleted(subscription) {
  const customerId = subscription.customer;
  db().prepare('UPDATE licenses SET revoked = 1, tier = ?, updated_at = ? WHERE stripe_customer_id = ?').run('free', Date.now(), customerId);
  console.log(' -> Subscription deleted, downgraded to free');
}

async function handleInvoicePaid(invoice) {
  const customerId = invoice.customer;
  db().prepare('UPDATE licenses SET expires_at = ?, revoked = 0, updated_at = ? WHERE stripe_customer_id = ?').run(
