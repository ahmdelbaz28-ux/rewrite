<div align="center">

<img src="docs/assets/smartlangguard-logo.svg" alt="SmartLangGuard Logo" width="120" />

# SmartLangGuard

**ßÊÈÊ ÅäÌáíÒí æÇäÊ äÇæí ÚÑÈí¿ ÇáÃÏÇÉ Ïí ÈÊÕáÍåÇ áß İí ÌÒÁ ãä ÇáËÇäíÉ.**

`high hofhv;` ? `ÇåáÇ ÇÎÈÇÑß` ?

[![npm version](https://img.shields.io/npm/v/@smartlangguard/cli.svg?style=flat-square&label=CLI&color=blue)](https://www.npmjs.com/package/@smartlangguard/cli)
[![Tests](https://img.shields.io/badge/Tests-240%20passing-brightgreen.svg?style=flat-square)](https://github.com/ahmdelbaz28-ux/rewrite/actions)
[![Version](https://img.shields.io/badge/Version-0.1.5-blue.svg?style=flat-square)](https://github.com/ahmdelbaz28-ux/rewrite/releases/tag/v0.1.5)
[![License](https://img.shields.io/badge/License-Proprietary-red.svg?style=flat-square)](LICENSE)
[![Platform](https://img.shields.io/badge/Win%20|%20macOS%20|%20Linux-lightgrey.svg?style=flat-square)](#-installation)

**[?? ÊäÒíá ÇáÅÕÏÇÑ 0.1.5](https://github.com/ahmdelbaz28-ux/rewrite/releases/tag/v0.1.5)** · **[npm](https://www.npmjs.com/package/@smartlangguard/cli)** · **[ÇáÓÌá](CHANGELOG.md)**

</div>

---

## ? ÂÎÑ ÇáÊÍÏíËÇÊ (ÇáäÓÎÉ 0.1.5)

| ÇáÊÍÏíË | ÇáæÕİ |
|---------|--------|
| ?? **240 ÇÎÊÈÇÑ äÇÌÍ** | ßá ÇáÈÇßÏÌÇÊ Êã ÇÎÊÈÇÑåÇ Úáì Node 18/20/22 Úáì GitHub Actions |
| ?? **ÅÕáÇÍ hang İí MCP Server** | ÇáÜ readline ÈŞí íÊåíøÇ Ìæøå `startMcpServer()` ÈÏá ãÇ íÚáøŞ ÇáÌÓÊ |
| ?? **ÅÕáÇÍ ÈäÇÁ CLI** | ÔíáäÇ conflict Èíä `--out-path` æ`--output` İí @yao-pkg/pkg |
| ?? **ÓßÑÈÊÇÊ VS Code ãÍãæáÉ** | ÇÓÊÈÏáäÇ ÇáãÓÇÑ ÇáËÇÈÊ `/home/z/...` ÈÓßÑÈÊ Ìæøå ÇáÈÇßÏÌ äİÓå |
| ?? **ÓßÑÈÊÇÊ Browser Extension** | ÃäÔÃäÇ `scripts/build.js` æ`scripts/package.js` Çááí ßÇäÊ äÇŞÕÉ |
| ?? **Artifacts ÌÇåÒÉ ááÊÍãíá** | CLI binary (Linux x64) + VSIX + ZIP İí [ÕİÍÉ ÇáÅÕÏÇÑÇÊ](https://github.com/ahmdelbaz28-ux/rewrite/releases/tag/v0.1.5) |

---

## ? ããíÒÇÊ ÇáäÓÎÉ 0.2.0

| ÇáãíÒÉ | ÇáæÕİ |
|--------|--------|
| ?? **ßÔİ ÇáäÕæÕ ÇáãÔÑæÚÉ** | URLs, ÅíãíáÇÊ, ÃßæÇÏ, ãÓÇÑÇÊ ãáİÇÊ¡ ÃáæÇä Hex¡ ÃÑŞÇã IP — ßáåÇ ãÊÊÛíÑÔ |
| ?? **ÏÚã 3 ÃäæÇÚ ßíÈæÑÏ** | QWERTY (ÃãÑíßí), AZERTY (İÑäÓí), QWERTZ (ÃáãÇäí) |
| ?? **ŞÇãæÓ ãÓÊÎÏã Ğßí** | ÇáÃÏÇÉ ÈÊÊÚáã ãä ÑİÖß ááÊäÈíåÇÊ — ÈÚÏ 3 ãÑÇÊ ÈÊÖíİ ÇáßáãÉ ÊáŞÇÆí |
| ?? **ÃãÇä ÇáÜ Daemon** | ÑãÒ ãÕÇÏŞÉ (Token) áßá ØáÈ API + CORS ãŞíÏ |
| ?? **ÊÔÛíá ÊáŞÇÆí** | ÇÎÊíÇÑí: ÔÛøá ÇáÃÏÇÉ ãÚ ÈÏÁ ÊÔÛíá Windows/macOS/Linux |
| ?? **ÅÔÚÇÑÇÊ Toast** | ÅÔÚÇÑÇÊ ÛíÑ ãÒÚÌÉ İí ÇáÎáİíÉ ÈÏá ÇáÑÓÇÆá ÇáãäÈËŞÉ |
| ?? **ŞÇãæÓ ÚÑÈí ãæÓøÚ** | ÃßÊÑ ãä 500 ßáãÉ + ÇááåÌÇÊ: ÇáãÕÑíÉ¡ ÇáÎáíÌíÉ¡ ÇáÔÇãíÉ |
| ?? **ÊÍáíá ÕÑİí áÇÕŞ** | ßÔİ ÇáÌĞæÑ ÇáÚÑÈíÉ ãÚ ÇáÈÇÏÆÇÊ æÇááæÇÍŞ (æÈÇáßÊÈ ? ßÊÈ) |
| ?? **ßÔİ ÇáÓíÇŞ** | ÇáÃÏÇÉ ÈÊİåã áæ ÅäÊ ÈÊßÊÈ ÚÑÈí Ãæ ÅäÌáíÒí ãä ÇáßáãÇÊ ÇáÓÇÈŞÉ |

---

## Åíå ÇáÍßÇíÉ¿

ÊÎíá Åäß ÈÊßÊÈ ÚÑÈí¡ áßä äÓíÊ ÊÛíøÑ áÛÉ ÇáßíÈæÑÏ ãä ÅäÌáíÒí áÚÑÈí.  
ÈÏá ãÇ ÊßÊÈ **"ÇåáÇ ÇÎÈÇÑß"** ØáÚÊ ãäß **"high hofhv;"** — æÏå ÛáØ ãÔ İí ÇáÅãáÇÁ¡ Ïå ÛáØ İí **áÛÉ ÇáßíÈæÑÏ**.

**SmartLangGuard** ÈíßÊÔİ ÇáÛáØÉ Ïí **ÃæÊæãÇÊíß** æíÕáÍåÇ áß ãä ÛíÑ ãÇ ÊÚãá Ãí ÍÇÌÉ íÏæí.

### ÇáİßÑÉ ÈÈÓÇØÉ

ßá ÒÑ İí ÇáßíÈæÑÏ ÈíáÚÈ ÏæÑíä:

| áãÇ ÇáßíÈæÑÏ **ÅäÌáíÒí** | äİÓ ÇáÒÑ áãÇ ÇáßíÈæÑÏ **ÚÑÈí** |
|:---:|:---:|
| `h` | `Ç` |
| `i` | `å` |
| `g` | `á` |
| `h` | `Ç` |

íÚäí **`high`** (ÈÇáÅäÌáíÒí) = **`ÇåáÇ`** (ÈÇáÚÑÈí) — äİÓ ÇáÃÒÑÇÑ ÈÇáÙÈØ!

---

## ÇáãÊØáÈÇÊ (ŞÈá ãÇ ÊÈÏÃ)

### 1. Node.js (ãÍÑß JavaScript)

ÇáÃÏÇÉ ãÍÊÇÌÉ **Node.js** ÚÔÇä ÊÔÊÛá. Ïå ÈÑäÇãÌ ãÌÇäí ÈÊËÈÊå ãÑÉ æÇÍÏÉ.

**ÅÒÇí ÃÊÃßÏ Åä ÚäÏí Node.js¿**

ÇİÊÍ **PowerShell** Ãæ **Terminal** æÇßÊÈ:
```bash
node --version
```

- áæ ÙåÑ ÑŞã Òí `v18.20.0` Ãæ ÃÚáì ? ÊãÇã¡ ÚäÏß Node.js
- áæ ÙåÑ ÎØÃ ? ãÍÊÇÌ ÊËÈÊå

**ÅÒÇí ÃËÈøÊ Node.js¿**

1. ÑæÍ ááãæŞÚ: [https://nodejs.org](https://nodejs.org)
2. ÇÖÛØ Úáì ÇáÒÑ ÇáÃÎÖÑ ÇáßÈíÑ **"Download Node.js"** (ÇÎÊÇÑ LTS)
3. ÔÛøá Çáãáİ Çááí äÒá æÇÖÛØ **Next ? Next ? Install ? Finish**
4. ÇİÊÍ PowerShell ÊÇäí æÇßÊÈ `node --version` — ÇáãİÑæÖ íÙåÑ ÇáÑŞã

### 2. ÊäÒíá ÇáÃÏÇÉ

ÚäÏß 3 ØÑŞ¡ ÇÎÊÇÑ Çááí íäÇÓÈß:

**ÇáØÑíŞÉ ÇáÃæáì: npm (ÇáÃÓåá æÇáÃÓÑÚ) ?**
```bash
npm install -g @smartlangguard/cli
```
ÈÚÏ ßÏå ÊŞÏÑ ÊÓÊÎÏãåÇ ãä Ãí ãßÇä ÈÇáÃãÑ `smartlangguard` Ãæ `slg`.

**ÇáØÑíŞÉ ÇáËÇäíÉ: ÊÍãíá Binary ÌÇåÒ (ãä ÛíÑ Node.js)**
1. ÑæÍ áÜ [ÕİÍÉ ÇáÅÕÏÇÑÇÊ](https://github.com/ahmdelbaz28-ux/rewrite/releases/latest)
2. äÒøá Çáãáİ ÇáãäÇÓÈ áÌåÇÒß:
   - **Linux:** `smartlangguard-cli-0.1.5-linux-x64`
   - **Windows:** áãÇ ÇáÅÕÏÇÑ íØáÚ ÑÓãíÇğ äÒøá ãä Release
3. **Linux/Mac:** ÇİÊÍ Terminal æäİøĞ:
   ```bash
   chmod +x smartlangguard-cli-0.1.5-linux-x64
   sudo mv smartlangguard-cli-0.1.5-linux-x64 /usr/local/bin/smartlangguard
   ```
4. ÌÑøÈ: `smartlangguard --version`

**ÇáØÑíŞÉ ÇáËÇáËÉ: ãä ÇáßæÏ ÇáãÕÏÑí (ááãØæÑíä)**
```bash
git clone https://github.com/ahmdelbaz28-ux/rewrite.git
cd rewrite
npm install
```

Ãæ Íãøá ZIP:
1. ÑæÍ áÜ [https://github.com/ahmdelbaz28-ux/rewrite](https://github.com/ahmdelbaz28-ux/rewrite)
2. ÇÖÛØ **Code** (ÇáÒÑ ÇáÃÎÖÑ) ? **Download ZIP**
3. İß ÇáÖÛØ Úä Çáãáİ
4. ÇİÊÍ PowerShell/Terminal ÏÇÎá ÇáãÌáÏ æÇßÊÈ:
```bash
npm install
```

> ?? `npm install` ÈÊÇÎÏ ÏŞíŞÉ Ãæ ÇÊäíä. ÈÊÍãøá ßá ÇáãßÊÈÇÊ Çááí ÇáãÔÑæÚ ãÍÊÇÌåÇ.

---

## 10 ØÑŞ ÊÓÊÎÏã ÈíåÇ ÇáÃÏÇÉ

---

### ÇáØÑíŞÉ 1: ÓØÑ ÇáÃæÇãÑ (CLI) — ÃÈÓØ ØÑíŞÉ

> **ãäÇÓÈÉ áÜ:** ÊÕÍíÍ äÕ ãÚíä ÈÓÑÚÉ ãä Ãí ãßÇä

**ÎØæÉ ÈÎØæÉ:**

**1.** ÇİÊÍ **PowerShell** (Windows) Ãæ **Terminal** (Mac/Linux)
   - **Windows:** ÇÖÛØ Úáì ßíÈæÑÏ `Win + R`¡ ÇßÊÈ `powershell`¡ ÇÖÛØ Enter
   - **Mac:** ÇİÊÍ ÊØÈíŞ **Terminal**

**2.** ÑæÍ ááãÌáÏ ÈÊÇÚ ÇáãÔÑæÚ:
```bash
cd c:\Users\EWS-01\Documents\Qoder\2026-06-22\chat-1\rewrite
```
> ÛíøÑ ÇáãÓÇÑ Ïå ÍÓÈ ãßÇä ÇáãÔÑæÚ ÚäÏß

**3.** ÇßÊÈ ÇáÃãÑ Ïå áÊÕÍíÍ äÕ:
```bash
node packages/cli/bin/smartlangguard.js fix "high hofhv;"
```

**ÇáäÊíÌÉ:**
```
ÇåáÇ ÇÎÈÇÑß
```

**ÃãËáÉ ÅÖÇİíÉ:**

```bash
# ßáãÉ æÇÍÏÉ
node packages/cli/bin/smartlangguard.js fix "high"
# ÇáäÊíÌÉ: ÇåáÇ

# ÈÇáÚßÓ (ÚÑÈí ? ÅäÌáíÒí) áæ ÇáßíÈæÑÏ ßÇä ÚÑÈí æÇäÊ äÇæí ÅäÌáíÒí
node packages/cli/bin/smartlangguard.js fix "ÇåáÇ" -d ar-to-en
# ÇáäÊíÌÉ: high

# ãÚ ÊİÇÕíá ÃßÊÑ (äÓÈÉ ÇáËŞÉ + ÇáÇÊÌÇå)
node packages/cli/bin/smartlangguard.js fix "high" --format text-with-meta
# ÇáäÊíÌÉ:
# ÇåáÇ
# [direction: en-to-ar | confidence: 90% | source: rules]

# ÈÕíÛÉ JSON (ãİíÏÉ ááÈÑãÌÉ)
node packages/cli/bin/smartlangguard.js fix "high" --format json
```

---

### ÇáØÑíŞÉ 2: ÇáæÖÚ ÇáÊİÇÚáí (Interactive) — Òí ÔÇÊ ÈæÊ

> **ãäÇÓÈÉ áÜ:** áæ ÚÇíÒ ÊÕÍøÍ ÃßÊÑ ãä äÕ æÑÇ ÈÚÖ

**ÎØæÉ ÈÎØæÉ:**

**1.** ÇİÊÍ PowerShell æÑæÍ áãÌáÏ ÇáãÔÑæÚ (Òí ÇáØÑíŞÉ 1)

**2.** ÇßÊÈ:
```bash
node packages/cli/bin/smartlangguard.js interactive
```

**åÊÙåÑ áß:**
```
SmartLangGuard Interactive Mode
Type text and press Enter to fix. Type "exit" or Ctrl+D to quit.

smartlangguard>
```

**3.** ÇßÊÈ Ãí äÕ æÇÖÛØ **Enter**:
```
smartlangguard> high
? ÇåáÇ
  [en-to-ar | 90% confidence | rules]
```

**4.** ÇßÊÈ ÊÇäí:
```
smartlangguard> hofhv;
? ÇÎÈÇÑß
  [en-to-ar | 85% confidence | rules]
```

**5.** áãÇ ÊÎáÕ¡ ÇßÊÈ `exit`:
```
smartlangguard> exit
Goodbye!
```

---

### ÇáØÑíŞÉ 3: ÇßÊÔÇİ ÇáÃÎØÇÁ ãä ÛíÑ ÊÕÍíÍ (Detect)

> **ãäÇÓÈÉ áÜ:** áæ ÚÇíÒ ÊÔæİ ÇáÃÎØÇÁ İíä ÈÇáÙÈØ ãä ÛíÑ ãÇ ÇáÃÏÇÉ ÊÚÏøá

**ÎØæÉ ÈÎØæÉ:**

```bash
node packages/cli/bin/smartlangguard.js detect "high hofhv;"
```

**ÇáäÊíÌÉ:**
```
Found 2 mistake(s):

  1. "high" ? "ÇåáÇ" (en-to-ar) [pos 0-4]
  2. "hofhv;" ? "ÇÎÈÇÑß" (en-to-ar) [pos 5-11]
```

> ÈíŞæáß: áŞíÊ ÛáØÊíä. ÇáÃæáì "high" ÇáãİÑæÖ Êßæä "ÇåáÇ"¡ æÇáÊÇäíÉ "hofhv;" ÇáãİÑæÖ Êßæä "ÇÎÈÇÑß"

**ÈÕíÛÉ JSON:**
```bash
node packages/cli/bin/smartlangguard.js detect "high hofhv;" --format json
```

---

### ÇáØÑíŞÉ 4: ÊÕÍíÍ ãáİ ßÇãá

> **ãäÇÓÈÉ áÜ:** áæ ÚäÏß ãáİ äÕí (txt, md) æİíå ßáÇã ãßÊæÈ ÈÇáßíÈæÑÏ ÇáÛáØ

**ÎØæÉ ÈÎØæÉ:**

**1.** ÌåøÒ ãáİ ÇÓãå `myfile.txt` æÍØø İíå ÇáäÕ ÇáÛáØ¡ ãËáÇ:
```
high hofhv;
```

**2.** ÇßÊÈ ÇáÃãÑ:
```bash
node packages/cli/bin/smartlangguard.js fix -f myfile.txt -o fixed.txt
```

**3.** ÇİÊÍ ãáİ `fixed.txt` — åÊáÇŞí:
```
ÇåáÇ ÇÎÈÇÑß
```

---

### ÇáØÑíŞÉ 5: ãä ÎáÇá Pipe (ÃæÊæãÇÊíß ãä ÈÑÇãÌ ÊÇäíÉ)

> **ãäÇÓÈÉ áÜ:** áæ ÚÇíÒ ÊÑÈØ ÇáÃÏÇÉ ãÚ ÃÏæÇÊ ÊÇäíÉ Ãæ ÓßÑíÈÊÇÊ

```bash
# Úáì Windows (PowerShell)
echo "high" | node packages/cli/bin/smartlangguard.js fix
# ÇáäÊíÌÉ: ÇåáÇ

# Úáì Mac/Linux
echo "high" | node packages/cli/bin/smartlangguard.js fix
# ÇáäÊíÌÉ: ÇåáÇ
```

---

### ÇáØÑíŞÉ 6: Daemon (ÎÏãÉ İí ÇáÎáİíÉ) — ? ÇáÃäÓÈ ááãÓÊÎÏã ÇáÚÇÏí

> **ãäÇÓÈÉ áÜ:** ÊÔÛíá ÇáÃÏÇÉ İí ÇáÎáİíÉ ÚÔÇä ÊÑÇŞÈ ÇáÜ Clipboard æÊÕáÍ Ãí äÕ ÊÚãá áå Copy

**ÇáãíÒÉ ÇáßÈíÑÉ:** ÈÊÔÊÛá İí ÇáÎáİíÉ æÊÑÇŞÈ ßá ÍÇÌÉ ÈÊÚãáåÇ Copy. æáæ ÖÛØÊ `Ctrl+Shift+Space`¡ ÈÊÕáÍ ÇáÜ Clipboard İæÑğÇ.

**ÎØæÉ ÈÎØæÉ:**

**1.** ÇİÊÍ PowerShell æÑæÍ áãÌáÏ ÇáãÔÑæÚ

**2.** ÔÛøá ÇáÜ Daemon:
```bash
node packages/daemon/src/daemon.js
```

**åÊÙåÑ áß:**
```
+--------------------------------------------+
|  SmartLangGuard Daemon v0.2.0              |
|--------------------------------------------|
|  + Clipboard monitor: ACTIVE               |
|  + Global hotkey: Ctrl+Shift+Space         |
|  + Local API: http://localhost:41783        |
|  + Auth token: 34867282...                 |
+--------------------------------------------+
Press Ctrl+C to stop.
```

> ?? **ãåã:** 
> - Îáí ÇáÜ Terminal Ïå ãİÊæÍ. áæ ŞİáÊå¡ ÇáÜ Daemon åíŞİ.
> - ÇáÜ **Auth Token** Ïå áÇÒã íÊÈÚÊ ãÚ ßá ØáÈ ááÜ API (ÇáÃãÇä)

**3.** ÌÑøÈ:
   - ÇäÓÎ Ãí äÕ ÅäÌáíÒí (ãËáÇ: `high hofhv;`)
   - ÇÖÛØ **`Ctrl+Shift+Space`**
   - ÇÚãá **Paste** (`Ctrl+V`) — åÊáÇŞí ÇáäÕ ÇÊÕáÍ!

**4.** ÎíÇÑÇÊ ÅÖÇİíÉ:
```bash
# ÊÔÛíá ãÚ ÈÏÁ ÇáÊÔÛíá ÇáÊáŞÇÆí
node packages/daemon/src/daemon.js --auto-start

# ÅíŞÇİ ÇáÊÔÛíá ÇáÊáŞÇÆí
node packages/daemon/src/daemon.js --disable-auto-start

# ãä ÛíÑ ãÑÇŞÈÉ Clipboard
node packages/daemon/src/daemon.js --no-clipboard
```

**5.** ÅíŞÇİ ÇáÜ Daemon: ÇÖÛØ `Ctrl+C` İí ÇáÜ Terminal

---

### ÇáØÑíŞÉ 7: ÅÖÇİÉ ÇáãÊÕİÍ (Browser Extension)

> **ãäÇÓÈÉ áÜ:** áæ ÈÊßÊÈ ßÊíÑ İí ÇáãÊÕİÍ (Facebook, Twitter, Gmail, WhatsApp Web...)

**ÇáÃÏÇÉ ÈÊÚãá Åíå İí ÇáãÊÕİÍ:**
- ÈÊßÊÔİ æÇäÊ ÈÊßÊÈ áæ ÇáßíÈæÑÏ ÛáØ
- ÈÊÏíß ÕæÊ ÊäÈíå
- ÈÊÕáÍ ÇáäÕ ÈÖÛØÉ æÇÍÏÉ: `Ctrl+Shift+Backspace`

**ÎØæÉ ÈÎØæÉ:**

**1.** ÔÛøá ÇáÜ Daemon ÇáÃæá (ÇáØÑíŞÉ 6) — Îáíå ÔÛÇá İí ÇáÎáİíÉ

**2.** ÇİÊÍ **Chrome** æÇßÊÈ İí ÔÑíØ ÇáÚäæÇä:
```
chrome://extensions
```

**3.** İÚøá **Developer mode** (ÇáãİÊÇÍ İí ÃÚáì íãíä ÇáÕİÍÉ)

**4.** ÇÖÛØ **Load unpacked**

**5.** ÇÎÊÇÑ ÇáãÌáÏ Ïå:
```
packages/browser-extension
```
(ÏÇÎá ãÌáÏ ÇáãÔÑæÚ)

**6.** åÊáÇŞí ÃíŞæäÉ SmartLangGuard ÙåÑÊ İí ÔÑíØ ÇáÃÏæÇÊ ?

**ÅÒÇí ÊÓÊÎÏãåÇ:**

| ÚÇíÒ ÊÚãá Åíå¿ | ÅÒÇí |
|---|---|
| ÊÕÍøÍ äÕ ãÍÏøÏ | ÍÏÏ ÇáäÕ ? ßáíß íãíä ? **SmartLangGuard: Fix Selection** |
| ÇÎÊÕÇÑ ÓÑíÚ | ÍÏÏ ÇáäÕ ? `Ctrl+Shift+L` |
| ÊÕÍøÍ ÂÎÑ ßáãÉ ßÊÈÊåÇ | æÇäÊ ÈÊßÊÈ ? `Ctrl+Shift+Backspace` |
| ÊİÊÍ ÇáÅÚÏÇÏÇÊ | ÇÖÛØ Úáì ÇáÃíŞæäÉ ? **Settings** |
| ÊİÊÍ ÇáäÇİĞÉ ÇáãäÈËŞÉ | ÇÖÛØ Úáì ÃíŞæäÉ SmartLangGuard İí ÔÑíØ ÇáÃÏæÇÊ |

**áæ ÙåÑÊ ÑÓÇáÉ "Daemon offline":**
- ÊÃßÏ Åä ÇáÜ Daemon ÔÛÇá (ÇáØÑíŞÉ 6)
- ÇÖÛØ Úáì ÇáÃíŞæäÉ ? áÇÒã ÊÙåÑ "Daemon running"

---

### ÇáØÑíŞÉ 8: ÅÖÇİÉ VS Code

> **ãäÇÓÈÉ áÜ:** áæ ÈÊßÊÈ ßæÏ Ãæ ãáÇÍÙÇÊ İí VS Code æÚÇíÒ ÊÕÍíÍ ÊáŞÇÆí

**ÎØæÉ ÈÎØæÉ:**

**1.** ËÈøÊ ÇáÜ CLI ÚÇáãíğÇ:
```bash
npm install -g @smartlangguard/cli
```

**2.** ÇİÊÍ **VS Code**

**3.** ÇÖÛØ `Ctrl+Shift+X` (ÚÔÇä ÊİÊÍ ŞÓã ÇáÅÖÇİÇÊ)

**4.** ÏæøÑ Úáì `SmartLangGuard` æÇÖÛØ **Install**

> áæ ãÔ áÇŞíåÇ¡ ããßä ÊÚãá Build ãä ÇáßæÏ:
> ```bash
> cd packages/vscode-extension
> npm run compile
> ```

**5.** ÈÚÏ ÇáÊËÈíÊ¡ ÇáÃÏÇÉ ÈÊÔÊÛá ÃæÊæãÇÊíß:

| ÚÇíÒ ÊÚãá Åíå¿ | ÅÒÇí |
|---|---|
| ÊÕÍøÍ ÇáäÕ ÇáãÍÏÏ | ÍÏÏ ÇáäÕ ? `Ctrl+Shift+P` ? ÇßÊÈ "Fix Selection" |
| ÊÕÍøÍ ÂÎÑ ßáãÉ | `Ctrl+Shift+Backspace` |
| ÊÔæİ ÇáÍÇáÉ | ÇÖÛØ Úáì "SmartLangGuard" İí ÔÑíØ ÇáÍÇáÉ (ÊÍÊ) |
| ÊÛíøÑ ÇáÕæÊ | `Ctrl+Shift+P` ? "SmartLangGuard: Sound Selection" |

---

### ÇáØÑíŞÉ 9: MCP Server (ãÚ ÃÏæÇÊ ÇáĞßÇÁ ÇáÇÕØäÇÚí)

> **ãäÇÓÈÉ áÜ:** áæ ÈÊÓÊÎÏã Claude Desktop Ãæ Cursor Ãæ Cline æÚÇíÒ ÇáĞßÇÁ ÇáÇÕØäÇÚí íŞÏÑ íÕÍøÍ ÇáäÕ

**ÎØæÉ ÈÎØæÉ:**

**1.** ËÈøÊ ÇáÜ CLI:
```bash
npm install -g @smartlangguard/cli
```

**2.** ÇİÊÍ ãáİ ÅÚÏÇÏÇÊ Claude Desktop:
   - **Windows:** `C:\Users\YOUR_NAME\AppData\Roaming\Claude\claude_desktop_config.json`
   - **Mac:** `~/Library/Application Support/Claude/claude_desktop_config.json`

**3.** ÃÖİ ÇáßæÏ Ïå:
```json
{
  "mcpServers": {
    "smartlangguard": {
      "command": "smartlangguard",
      "args": ["mcp"]
    }
  }
}
```

**4.** ÃÚÏ ÊÔÛíá Claude Desktop

**5.** ÏáæŞÊí Claude íŞÏÑ íÓÊÎÏã ÇáÃÏÇÉ. ÌÑÈ ÊŞæá áå:
> "Fix this text: high hofhv;"

æåæ ÈíÑÏ Úáíß ÈÜ: **"ÇåáÇ ÇÎÈÇÑß"**

---

### ÇáØÑíŞÉ 10: Node.js API (ááãÈÑãÌíä)

> **ãäÇÓÈÉ áÜ:** áæ ÚÇíÒ ÊÈäí ÊØÈíŞß ÇáÎÇÕ æÊÏãÌ ÇáÃÏÇÉ ÌæÇå

**ÎØæÉ ÈÎØæÉ:**

**1.** ËÈøÊ ÇáãßÊÈÉ İí ãÔÑæÚß:
```bash
npm install @smartlangguard/core
```

**2.** ÇßÊÈ ÇáßæÏ:
```javascript
const core = require('@smartlangguard/core');

// ÊåíÆÉ (ãÑÉ æÇÍÏÉ)
await core.init({ telemetryEnabled: false });

// ÊÕÍíÍ äÕ
const result = await core.fixText('high hofhv;');
console.log(result.corrected);  // ÇåáÇ ÇÎÈÇÑß
console.log(result.direction);  // en-to-ar
console.log(result.score);      // 88

// ÊÕÍíÍ ÈÇáÚßÓ
const result2 = await core.fixText('ÇåáÇ', { direction: 'ar-to-en' });
console.log(result2.corrected); // high
```

---

## ßá ÇáÃæÇãÑ İí ãßÇä æÇÍÏ

| ÇáÃãÑ | ÈíÚãá Åíå |
|-------|----------|
| `fix "äÕ"` | íÕÍøÍ ÇáäÕ |
| `fix -f ãáİ.txt -o äÊíÌÉ.txt` | íÕÍøÍ ãáİ æíÍØ ÇáäÊíÌÉ İí ãáİ ÊÇäí |
| `fix --format json "äÕ"` | íÕÍøÍ æíÚÑÖ ÇáäÊíÌÉ ÈÕíÛÉ JSON |
| `fix --format text-with-meta "äÕ"` | íÕÍøÍ æíÚÑÖ äÓÈÉ ÇáËŞÉ |
| `fix -d ar-to-en "ÇåáÇ"` | íÕÍøÍ ÈÇáÚßÓ (ÚÑÈí ? ÅäÌáíÒí) |
| `detect "äÕ"` | íßÊÔİ ÇáÃÎØÇÁ ãä ÛíÑ ãÇ íÕÍøÍ |
| `interactive` | æÖÚ ÊİÇÚáí (ÔÇÊ) |
| `daemon` | íÔÛøá ÎÏãÉ İí ÇáÎáİíÉ |
| `mcp` | íÔÛøá MCP server ááĞßÇÁ ÇáÇÕØäÇÚí |
| `license activate <ÑãÒ>` | íİÚøá ÑÎÕÉ |
| `license status` | íÚÑÖ ÍÇáÉ ÇáÑÎÕÉ |
| `config set telemetry false` | íæŞİ ÇáÊÊÈÚ |
| `update check` | íÏæÑ Úáì ÊÍÏíË |
| `sound play ding` | íÔÛøá ÕæÊ ÊäÈíå |
| `sound list` | íÚÑÖ ŞÇÆãÉ ÇáÃÕæÇÊ |

---

## ÇáããíÒÇÊ æÇáÈÇŞÇÊ

| ÇáãíÒÉ | ãÌÇäí | Pro ($5/ÔåÑ) |
|--------|:----:|:-----------:|
| ÊÕÍíÍ ÇáäÕæÕ | ? | ? |
| CLI + Daemon + Hotkey | ? | ? |
| ÅÖÇİÉ VS Code | ? | ? |
| ÅÖÇİÉ ÇáãÊÕİÍ | ? | ? |
| ßÔİ ÇáäÕæÕ ÇáãÔÑæÚÉ (URLs, ÃßæÇÏ) | ? | ? |
| ŞÇãæÓ ÇáãÓÊÎÏã ÇáĞßí | ? | ? |
| ÏÚã 3 ÃäæÇÚ ßíÈæÑÏ | ? | ? |
| ŞÇãæÓ ÚÑÈí 500+ ßáãÉ + áåÌÇÊ | ? | ? |
| ÊÔÛíá ÊáŞÇÆí ãÚ ÇáäÙÇã | ? | ? |
| ÊÕÍíÍ ÈÇáĞßÇÁ ÇáÇÕØäÇÚí (AI) | — | ? |
| ÚÏÏ ÇáÃÌåÒÉ | 1 | 3 |
| ÏÚã İäí | — | ? ÃæáæíÉ |

---

## ÇáÃÓÆáÉ ÇáÔÇÆÚÉ

### Ó: ÇáÃÏÇÉ ÈÊÈÚÊ ÈíÇäÇÊí áÃí ÍÏ¿
**áÇ.** ßá ÇáÊÕÍíÍ ÈíÊã **Úáì ÌåÇÒß**. ãİíÔ ÈíÇäÇÊ ÈÊÊÈÚÊ áÃí ÓíÑİÑ.

### Ó: ÈÊÔÊÛá offline¿
**ÃíæÇ.** ÈÚÏ ãÇ ÊËÈøÊ¡ ÇáÃÏÇÉ ÈÊÔÊÛá ãä ÛíÑ ÅäÊÑäÊ.

### Ó: ÈÊÏÚã áÛÇÊ ÊÇäíÉ ÛíÑ ÇáÚÑÈí æÇáÅäÌáíÒí¿
**áÃ ÍÇáíğÇ.** ÈÓ ÇáÚÑÈí æÇáÅäÌáíÒí. ããßä äÖíİ áÛÇÊ ÊÇäíÉ İí ÇáãÓÊŞÈá.

### Ó: ÅÒÇí ÃÚÑİ ÇáÃÏÇÉ ÔÛÇáÉ¿
ÌÑøÈ:
```bash
node packages/cli/bin/smartlangguard.js fix "high"
```
áæ ÇáäÊíÌÉ ØáÚÊ `ÇåáÇ` ? ÇáÃÏÇÉ ÔÛÇáÉ ÊãÇã ?

---

## ÇáÇÎÊÈÇÑÇÊ (Tests)

```bash
npm test                # 240 ÇÎÊÈÇÑ
npx jest --verbose      # ÊİÇÕíá ÃßÊÑ
```

### Åíå Çááí ÈíÊÎÊÈÑ¿
| ÇáæÍÏÉ | ÚÏÏ ÇáÇÎÊÈÇÑÇÊ | ÈÊÎÊÈÑ Åíå |
|--------|:-:|--------|
| Core (Translator, AI, License, …) | 146 | ÊÍæíá ÇáÍÑæİ¡ ßÔİ ÇáÃÎØÇÁ¡ ÇáäÕæÕ ÇáãÔÑæÚÉ¡ ŞÇãæÓ 500+ ßáãÉ¡ ÇáÊÍáíá ÇáÕÑİí |
| Backend API | 35 | License, Telemetry, Billing, Admin JWT, Stripe, 404 |
| Daemon | 14 | API, Auth Token, CORS, ÊÔÛíá ÈÏæä crash |
| CLI | 14 | --version, --help, fix (stdin/arg/file), detect, ßá ÕíÛ ÇáÅÎÑÇÌ |
| Browser Extension | 11 | MV3 manifest, ÇáÕáÇÍíÇÊ ÇáÂãäÉ, build + zip |
| Admin Dashboard | 9 | vite build + ÇáÍÒã ÇáÜ hashed |
| MCP Server | 6 | Protocol version, 4 tools (fix_text, fix_clipboard, register_license, license_status) |
| VS Code Extension | 5 | tsc compilation, exports activate/deactivate, portable paths |
| **ÇáÅÌãÇáí** | **240** | **ßá ÇáÈÇßÏÌÇÊ ÎÖÑÇÁ Úáì Node 18/20/22** |

---

## ÇáÊÑÎíÕ (License)

Proprietary — © 2026 SmartLangGuard.

[ÇáãÔÇßá æÇáÇŞÊÑÇÍÇÊ](https://github.com/ahmdelbaz28-ux/rewrite/issues) · [ÊæÇÕá ãÚäÇ](mailto:ahmdelbaz28@gmail.com)
<div align="center">

---

# ???? English Guide

---

## ? Latest Updates (v0.1.5)

| Update | Description |
|--------|-------------|
| ?? **240 passing tests** | All packages verified on Node 18/20/22 via GitHub Actions |
| ?? **Fixed MCP Server hang** | `readline` is now created inside `startMcpServer()` instead of at module-load time (was keeping the event loop alive) |
| ?? **Fixed CLI build** | Removed `--out-path` + `--output` conflict in @yao-pkg/pkg |
| ?? **Portable VS Code scripts** | Replaced the hardcoded `/home/z/...` path with a portable script inside the package |
| ?? **Browser Extension scripts** | Created the missing `scripts/build.js` and `scripts/package.js` |
| ?? **Downloadable artifacts** | CLI binary (Linux x64) + VSIX + ZIP available on the [releases page](https://github.com/ahmdelbaz28-ux/rewrite/releases/tag/v0.1.5) |

---

## ? v0.2.0 Features

| Feature | Description |
|-------------|-------------|
| ?? **False Positive Detection** | URLs, emails, code, file paths, hex colors, IP addresses are never touched |
| ?? **3 Keyboard Layouts** | QWERTY (US), AZERTY (French), QWERTZ (German) |
| ?? **Smart User Dictionary** | Auto-learns from dismissed alerts — after 3 dismissals, word is whitelisted |
| ?? **Daemon Security** | Token-based API authentication + restricted CORS |
| ?? **Auto-start on Login** | Optional: start SmartLangGuard when Windows/macOS/Linux boots |
| ?? **Toast Notifications** | Non-blocking background notifications instead of modal dialogs |
| ?? **500+ Arabic Words** | Expanded dictionary including Egyptian, Gulf, and Levantine dialects |
| ?? **Agglutinative Morphology** | Strips prefix + suffix to find Arabic roots (æÈÇáßÊÈ ? ßÊÈ) |
| ?? **Context Awareness** | Understands if you're typing Arabic or English from previous words |

---

## What's This?

Imagine you're typing in Arabic, but you forgot to switch your keyboard layout from English to Arabic.  
Instead of writing **"ÇåáÇ ÇÎÈÇÑß"**, you end up with **"high hofhv;"** — this isn't a spelling mistake, it's a **keyboard language** mistake.

**SmartLangGuard** detects this error **automatically** and fixes it without any manual effort.

### The Simple Idea

Every key on your keyboard plays two roles:

| When keyboard is set to **English** | Same key when keyboard is set to **Arabic** |
|:---:|:---:|
| `h` | `Ç` |
| `i` | `å` |
| `g` | `á` |
| `h` | `Ç` |

So **`high`** (typed in English) = **`ÇåáÇ`** (in Arabic) — same exact keys!

---

## Prerequisites (Before You Start)

### 1. Node.js (the JavaScript runtime)

SmartLangGuard needs **Node.js** to run. It's a free program you install once.

**How to check if you have Node.js:**

Open **PowerShell** (Windows) or **Terminal** (Mac/Linux) and type:
```bash
node --version
```

- If you see a number like `v18.20.0` or higher ? you're good to go
- If you see an error ? you need to install it

**How to install Node.js:**

1. Go to: [https://nodejs.org](https://nodejs.org)
2. Click the big green **"Download Node.js"** button (choose LTS)
3. Run the downloaded file and click **Next ? Next ? Install ? Finish**
4. Open PowerShell again and type `node --version` — you should now see the version number

### 2. Download the Tool

You have 3 options, pick whichever suits you:

**Option 1: npm (easiest and fastest) ?**
```bash
npm install -g @smartlangguard/cli
```
After that you can use it from anywhere via the `smartlangguard` or `slg` command.

**Option 2: Download a prebuilt binary (no Node.js required)**
1. Go to the [releases page](https://github.com/ahmdelbaz28-ux/rewrite/releases/latest)
2. Download the file that matches your machine:
   - **Linux:** `smartlangguard-cli-0.1.5-linux-x64`
   - **Windows:** once the official release is out, grab it from Releases
3. **Linux/Mac:** open Terminal and run:
   ```bash
   chmod +x smartlangguard-cli-0.1.5-linux-x64
   sudo mv smartlangguard-cli-0.1.5-linux-x64 /usr/local/bin/smartlangguard
   ```
4. Verify: `smartlangguard --version`

**Option 3: From source (for developers)**
```bash
git clone https://github.com/ahmdelbaz28-ux/rewrite.git
cd rewrite
npm install
```

Or download a ZIP:
1. Go to [https://github.com/ahmdelbaz28-ux/rewrite](https://github.com/ahmdelbaz28-ux/rewrite)
2. Click **Code** (green button) ? **Download ZIP**
3. Extract the ZIP file
4. Open PowerShell inside the extracted folder and run:
```bash
npm install
```

> ?? `npm install` takes a minute or two. It downloads all the libraries the project needs.

---

## 10 Ways to Use SmartLangGuard

---

### Method 1: Command Line (CLI) — the simplest way

> **Best for:** quickly fixing a piece of text from anywhere

**Step by step:**

**1.** Open **PowerShell** (Windows) or **Terminal** (Mac/Linux)
   - **Windows:** press `Win + R` on your keyboard, type `powershell`, press Enter
   - **Mac:** open the **Terminal** app

**2.** Navigate to the project folder:
```bash
cd path/to/your/rewrite/folder
```
> Change this path to wherever you extracted the project

**3.** Type this command to fix text:
```bash
node packages/cli/bin/smartlangguard.js fix "high hofhv;"
```

**Output:**
```
ÇåáÇ ÇÎÈÇÑß
```

**More examples:**

```bash
# Fix a single word
node packages/cli/bin/smartlangguard.js fix "high"
# Output: ÇåáÇ

# Reverse direction (Arabic ? English) if keyboard was Arabic but you meant English
node packages/cli/bin/smartlangguard.js fix "ÇåáÇ" -d ar-to-en
# Output: high

# Show more details (confidence + direction)
node packages/cli/bin/smartlangguard.js fix "high" --format text-with-meta
# Output:
# ÇåáÇ
# [direction: en-to-ar | confidence: 90% | source: rules]

# Output as JSON (useful for developers)
node packages/cli/bin/smartlangguard.js fix "high" --format json
```

---

### Method 2: Interactive Mode — like a chatbot

> **Best for:** fixing multiple texts one after another

**Step by step:**

**1.** Open PowerShell and navigate to the project folder (like Method 1)

**2.** Run:
```bash
node packages/cli/bin/smartlangguard.js interactive
```

**You'll see:**
```
SmartLangGuard Interactive Mode
Type text and press Enter to fix. Type "exit" or Ctrl+D to quit.

smartlangguard>
```

**3.** Type any text and press **Enter**:
```
smartlangguard> high
? ÇåáÇ
  [en-to-ar | 90% confidence | rules]
```

**4.** Type another:
```
smartlangguard> hofhv;
? ÇÎÈÇÑß
  [en-to-ar | 85% confidence | rules]
```

**5.** When done, type `exit`:
```
smartlangguard> exit
Goodbye!
```

---

### Method 3: Detect Mistakes Without Fixing (Detect)

> **Best for:** finding exactly where the mistakes are without making changes

**Step by step:**

```bash
node packages/cli/bin/smartlangguard.js detect "high hofhv;"
```

**Output:**
```
Found 2 mistake(s):

  1. "high" ? "ÇåáÇ" (en-to-ar) [pos 0-4]
  2. "hofhv;" ? "ÇÎÈÇÑß" (en-to-ar) [pos 5-11]
```

> It tells you: found 2 mistakes. The first "high" should be "ÇåáÇ", and "hofhv;" should be "ÇÎÈÇÑß"

**As JSON:**
```bash
node packages/cli/bin/smartlangguard.js detect "high hofhv;" --format json
```

---

### Method 4: Fix an Entire File

> **Best for:** if you have a text file (.txt, .md) with text typed in the wrong keyboard layout

**Step by step:**

**1.** Create a file called `myfile.txt` and put the wrong text in it, for example:
```
high hofhv;
```

**2.** Run:
```bash
node packages/cli/bin/smartlangguard.js fix -f myfile.txt -o fixed.txt
```

**3.** Open `fixed.txt` — you'll find:
```
ÇåáÇ ÇÎÈÇÑß
```

---

### Method 5: Pipe (automatic use from other programs)

> **Best for:** connecting the tool with other tools or scripts

```bash
# On Windows (PowerShell)
echo "high" | node packages/cli/bin/smartlangguard.js fix
# Output: ÇåáÇ

# On Mac/Linux
echo "high" | node packages/cli/bin/smartlangguard.js fix
# Output: ÇåáÇ
```

---

### Method 6: Daemon (background service) — ? best for everyday users

> **Best for:** running the tool in the background to monitor your clipboard and fix any text you copy

**The big advantage:** it runs in the background and watches everything you copy. If you press `Ctrl+Shift+Space`, it fixes the clipboard instantly.

**Step by step:**

**1.** Open PowerShell and navigate to the project folder

**2.** Start the Daemon:
```bash
node packages/daemon/src/daemon.js
```

**You'll see:**
```
+--------------------------------------------+
|  SmartLangGuard Daemon v0.2.0              |
|--------------------------------------------|
|  + Clipboard monitor: ACTIVE               |
|  + Global hotkey: Ctrl+Shift+Space         |
|  + Local API: http://localhost:41783        |
|  + Auth token: 34867282...                 |
+--------------------------------------------+
Press Ctrl+C to stop.
```

> ?? **Important:**
> - Keep this terminal window open. If you close it, the Daemon stops.
> - The **Auth Token** must be sent with every API request (security)

**3.** Try it:
   - Copy any English text (e.g., `high hofhv;`)
   - Press **`Ctrl+Shift+Space`**
   - Paste (`Ctrl+V`) — you'll see the text is now fixed!

**4.** Extra options:
```bash
# Start with auto-start on login enabled
node packages/daemon/src/daemon.js --auto-start

# Disable auto-start
node packages/daemon/src/daemon.js --disable-auto-start

# Without clipboard monitoring
node packages/daemon/src/daemon.js --no-clipboard
```

**5.** To stop the Daemon: press `Ctrl+C` in the terminal

---

### Method 7: Browser Extension

> **Best for:** if you write a lot in the browser (Facebook, Twitter, Gmail, WhatsApp Web...)

**What the extension does in the browser:**
- Detects while you type if the keyboard layout is wrong
- Plays an alert sound
- Fixes text with one keystroke: `Ctrl+Shift+Backspace`

**Step by step:**

**1.** Start the Daemon first (Method 6) — keep it running in the background

**2.** Open **Chrome** and type in the address bar:
```
chrome://extensions
```

**3.** Turn on **Developer mode** (toggle in the top-right corner)

**4.** Click **Load unpacked**

**5.** Select this folder:
```
packages/browser-extension
```
(inside the project folder)

**6.** You'll see the SmartLangGuard icon appear in the toolbar ?

**How to use it:**

| What you want to do | How |
|---|---|
| Fix selected text | Select text ? right-click ? **SmartLangGuard: Fix Selection** |
| Quick shortcut | Select text ? `Ctrl+Shift+L` |
| Fix the last word you typed | While typing ? `Ctrl+Shift+Backspace` |
| Open settings | Click the icon ? **Settings** |
| Open the popup | Click the SmartLangGuard icon in the toolbar |

**If you see "Daemon offline":**
- Make sure the Daemon is running (Method 6)
- Click the icon ? it should show "Daemon running"

---

### Method 8: VS Code Extension

> **Best for:** if you write code or notes in VS Code and want automatic correction

**Step by step:**

**1.** Install the CLI globally:
```bash
npm install -g @smartlangguard/cli
```

**2.** Open **VS Code**

**3.** Press `Ctrl+Shift+X` (to open the Extensions panel)

**4.** Search for `SmartLangGuard` and click **Install**

> If you can't find it, you can build from source:
> ```bash
> cd packages/vscode-extension
> npm run compile
> ```

**5.** After installation, the tool works automatically:

| What you want to do | How |
|---|---|
| Fix selected text | Select text ? `Ctrl+Shift+P` ? type "Fix Selection" |
| Fix last word | `Ctrl+Shift+Backspace` |
| Check status | Click "SmartLangGuard" in the status bar (bottom) |
| Change alert sound | `Ctrl+Shift+P` ? "SmartLangGuard: Sound Selection" |

---

### Method 9: MCP Server (with AI tools)

> **Best for:** if you use Claude Desktop, Cursor, or Cline and want AI to fix text for you

**Step by step:**

**1.** Install the CLI:
```bash
npm install -g @smartlangguard/cli
```

**2.** Open the Claude Desktop config file:
   - **Windows:** `C:\Users\YOUR_NAME\AppData\Roaming\Claude\claude_desktop_config.json`
   - **Mac:** `~/Library/Application Support/Claude/claude_desktop_config.json`

**3.** Add this configuration:
```json
{
  "mcpServers": {
    "smartlangguard": {
      "command": "smartlangguard",
      "args": ["mcp"]
    }
  }
}
```

**4.** Restart Claude Desktop

**5.** Now Claude can use the tool. Try asking:
> "Fix this text: high hofhv;"

And it will respond with: **"ÇåáÇ ÇÎÈÇÑß"**

---

### Method 10: Node.js API (for developers)

> **Best for:** if you want to build your own app and integrate the tool inside it

**Step by step:**

**1.** Install the library in your project:
```bash
npm install @smartlangguard/core
```

**2.** Write the code:
```javascript
const core = require('@smartlangguard/core');

// Initialize (once)
await core.init({ telemetryEnabled: false });

// Fix text
const result = await core.fixText('high hofhv;');
console.log(result.corrected);  // ÇåáÇ ÇÎÈÇÑß
console.log(result.direction);  // en-to-ar
console.log(result.score);      // 88

// Reverse direction
const result2 = await core.fixText('ÇåáÇ', { direction: 'ar-to-en' });
console.log(result2.corrected); // high
```

---

## All Commands in One Place

| Command | What it does |
|---------|-------------|
| `fix "text"` | Fix the text |
| `fix -f file.txt -o output.txt` | Fix a file and save the result to another file |
| `fix --format json "text"` | Fix and output the result as JSON |
| `fix --format text-with-meta "text"` | Fix and show the confidence level |
| `fix -d ar-to-en "ÇåáÇ"` | Reverse direction (Arabic ? English) |
| `detect "text"` | Find mistakes without fixing them |
| `interactive` | Interactive (chat) mode |
| `daemon` | Run as a background service |
| `mcp` | Run the MCP server for AI tools |
| `license activate <token>` | Activate a license |
| `license status` | Show license status |
| `config set telemetry false` | Disable telemetry |
| `update check` | Check for updates |
| `sound play ding` | Play an alert sound |
| `sound list` | List available sounds |

---

## Features & Pricing

| Feature | Free | Pro ($5/mo) |
|---------|:----:|:-----------:|
| Text correction | ? | ? |
| CLI + Daemon + Hotkey | ? | ? |
| VS Code Extension | ? | ? |
| Browser Extension | ? | ? |
| False positive detection (URLs, code) | ? | ? |
| Smart user dictionary | ? | ? |
| 3 keyboard layouts | ? | ? |
| 500+ word Arabic dictionary + dialects | ? | ? |
| Auto-start on login | ? | ? |
| AI-powered scoring | — | ? |
| Number of devices | 1 | 3 |
| Priority support | — | ? |

---

## Frequently Asked Questions

### Q: Does the tool send my data anywhere?
**No.** All correction happens **on your device**. No data is sent to any server.

### Q: Does it work offline?
**Yes.** Once installed, the tool works without internet.

### Q: Does it support languages other than Arabic and English?
**Not yet.** Only Arabic and English for now. Other languages may be added in the future.

### Q: How do I know the tool is working?
Try this:
```bash
node packages/cli/bin/smartlangguard.js fix "high"
```
If the output is `ÇåáÇ` ? the tool is working perfectly ?

---

## Tests

```bash
npm test                # 240 tests
npx jest --verbose      # more details
```

### What's tested?
| Module | Tests | What's tested |
|--------|:-:|--------|
| Core (Translator, AI, License, …) | 146 | char mapping, mistake detection, false positives, 500+ word dictionary, morphology |
| Backend API | 35 | License, Telemetry, Billing, Admin JWT, Stripe, 404 |
| Daemon | 14 | API, auth token, CORS, clean startup/shutdown |
| CLI | 14 | --version, --help, fix (stdin/arg/file), detect, all output formats |
| Browser Extension | 11 | MV3 manifest, safe permissions, build + zip packaging |
| Admin Dashboard | 9 | vite build + hashed bundles |
| MCP Server | 6 | protocol version, 4 tools (fix_text, fix_clipboard, register_license, license_status) |
| VS Code Extension | 5 | tsc compilation, exports activate/deactivate, portable paths |
| **Total** | **240** | **all green on Node 18/20/22** |

---

## License

Proprietary — © 2026 SmartLangGuard.

[Issues & Suggestions](https://github.com/ahmdelbaz28-ux/rewrite/issues) · [Contact Us](mailto:ahmdelbaz28@gmail.com)
